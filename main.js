...
const cars = generateCars(1000).filter( c => c.year > 1900)  // 1000 cars, all ages
let filtered = []
...
function bandButtons(property, numBands) {
    ...
    const b = bands(property, numBands)  ////b will be an array of band objects {from:x,to:y} - for the requested property of the cars (e.g. mileage, price)
...
function bands(property, numBands) {
    ...
    for (let i = 0; i < numBands; i++) {
        b.push(
            {
             from: Math.floor(min + (range / numBands) * i), 
             to: Math.floor(min + (range / numBands) * (i + 1)) 
            }
            )
    }
